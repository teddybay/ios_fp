...
class AddDiaryTableViewController: UITableViewController , UIImagePickerControllerDelegate,UINavigationControllerDelegate{
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let tap: UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(AddDiaryTableViewController.dismissKeyboard))
    ...
    @IBAction func done(_ sender: Any) {
        ...
        else if conceptTextView.text?.characters.count == 0
        {
            let controller = UIAlertController(title: "小提醒", message: "請輸入日記內容", preferredStyle: .alert)
            let action = UIAlertAction(title: "確定", style: .default, handler: { (action) in
        ...
        let data = UIImageJPEGRepresentation(Image.image!, 0.8)
        ...
        let diary = ["date":dateTextField.text!, "photo":"\(interval)", "concept":conceptTextView.text!]
        let notiName2 = Notification.Name("AddDiaryList")
        NotificationCenter.default.post(name: notiName2, object: nil, userInfo: diary)
